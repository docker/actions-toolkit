...
import {Cosign, CosignCommandResult} from '../cosign/cosign';
...
export interface SignAttestationManifestsResult extends CosignCommandResult {
  noTransparencyLog?: boolean;
  imageName: string;
}
...
export class Sigstore {
    ...
    try {
      ...
      for (const attestationDigest of attestationDigests) {
        ...
        await core.group(`Signing attestation manifest ${attestationRef}`, async () => {
          ...
          core.info(`Attestation manifest signed and signature manifest pushed: https://oci.dag.dev/?referrers=${attestationRef}`);
          result[attestationRef] = {
            ...signResult,
            noTransparencyLog: noTransparencyLog,
  ...
  public async verifySignedManifests(opts: VerifySignedManifestsOpts, signed: Record<string, SignAttestationManifestsResult>): Promise<Record<string, VerifySignedManifestsResult>> {
    ...
    for (const [attestationRef, signedRes] of Object.entries(signed)) {
      await core.group(`Verifying ${attestationRef}`, async () => {
        // prettier-ignore
        const cosignArgs = [
          '--verbose',
          'verify',
        ...
        if (signedRes.noTransparencyLog) {
        ...
        for (let attempt = 0; attempt < retries; attempt++) {
          ...
          if (execRes.exitCode === 0) {
            ...
            core.info(`Attestation manifest signature verified: https://oci.dag.dev/?image=${signedRes.imageName}@${verifyResult.signatureManifestDigest}`);
            break;
          } else {
            if (verifyResult.errors && verifyResult.errors.length > 0) {
              ...
              if (verifyResult.errors.some(e => e.code === 'MANIFEST_UNKNOWN')) {
                ...
                throw new Error(`Cosign verify command failed with errors:\n${errorMessages}`);
              }
            } else {
              throw new Error(`Cosign verify command failed with exit code ${execRes.exitCode}`);
            }
          }
        }
      });
