...
- mode: system
  script: |
    ...
    if [ "{{srcType}}" == "archive" ]; then
      ...
      sed -i 's|^ExecStart=.*|ExecStart=/usr/bin/dockerd -H fd:// -H tcp://0.0.0.0:2375 --containerd=/run/containerd/containerd.sock|' /usr/lib/systemd/system/docker.service
      ...
      sed -i 's|^ExecStart=.*|ExecStart=/usr/local/bin/dockerd -H fd:// -H tcp://0.0.0.0:2375|' /etc/systemd/system/docker.service
